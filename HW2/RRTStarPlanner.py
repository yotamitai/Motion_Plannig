...
class RRTStarPlanner(object):
    ...
    def Plan(self, start_config, goal_config, epsilon=0.001):
        ...
        epsilon = 20  # E1, E2 # 0 = extend all the way
        k = 8  # k = 8,5,
        log_n_flag = True
        normal_run = True
        verbose = False
        ...
        while new_vertex:
            iterations += 1
            if log_n_flag:
                k = int(round(numpy.log(iterations))) + 1
            rand_vertex, iterations = self.get_sample(seen, bais, goal_config, plan_improvement_flag, iterations)
            ...
            if self.collision_free(nearest_vertex, new_vertex):
                ...
                # Rewire
                self.rewire(k, seen, new_vertex, new_vertex_id, start_config)
            else:
                continue
    ...
    def rewire(self, k, seen, new_vertex, new_vertex_id, start_config):
        if len(seen) > k + 1:
            distances = []
            ...
            for neighbour_id in nearest_vertices_id:
                ...
                if self.collision_free(neighbour_coord, new_vertex):
                    distance = self.get_plan(start_config, neighbour_coord, neighbour_id)[1]
            ...
            # second direction
            new_distance = self.get_plan(start_config, new_vertex, new_vertex_id)[1]
            for _, n_id in distances:
                n_coord = self.tree.vertices[n_id]
                dist_to_new = self.planning_env.compute_distance(n_coord, new_vertex)
                distance = self.get_plan(start_config, n_coord, n_id)[1]
                if distance > new_distance + dist_to_new:
                    self.tree.AddEdge(new_vertex_id, n_id)
    ...
    def get_sample(self, seen, bais, goal_config, plan_improvement_flag, iterations):
        # goal_bias
